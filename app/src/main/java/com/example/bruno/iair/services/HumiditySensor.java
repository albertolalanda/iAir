package com.example.bruno.iair.services;
...
public class HumiditySensor extends Activity implements SensorEventListener {
    ...
    */
    ...
    ////TODO
    public boolean isHumidityAndTemperatureSensorPresent(){
        if(mSensorManager.getDefaultSensor(Sensor.TYPE_RELATIVE_HUMIDITY) != null) {
            mHumiditySensor = mSensorManager.getDefaultSensor(Sensor.TYPE_RELATIVE_HUMIDITY);
            isHumiditySensorPresent = true;
        }
    ...
    public double getRelativeHumidity(SensorEvent event) {
        if(event.sensor.getType()==Sensor.TYPE_RELATIVE_HUMIDITY) {
            return event.values[0];}
            return 0;}


    public double getTemperature(SensorEvent event) {
